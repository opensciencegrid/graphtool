    ...
    def draw( self ):
        ...
        for pivot in keys:
            ...
            if type(data) == types.TupleType:
                ...
                if len(data) == 3:
                    yerr2.append(float(data[2]))
            else:
                tmp_y.append( float(data) )
        if len(yerr) != 0:
            if not yerr2:
                self.bars = self.ax.barh(tmp_x, tmp_y, height=width, xerr=yerr,
                   ecolor='red')
            else:
                self.bars = self.ax.barh(tmp_x, tmp_y, height=width,
                    ecolor='red')
                self.errorbar(tmp_y, tmp_x, fmt=None, ecolor='r',
                    xerr=numpy.array([yerr, yerr2]))
        else:
            self.bars = self.ax.barh( tmp_x, tmp_y, height=width )
        setp( self.bars, linewidth=0.5 )
        pivots = keys
...
class StackedLineGraph(StackedBarGraph):
  ...
  def get_coords( self ):
    ...
    for pivot in keys:
      ...
      bottom_left = csum_left
      csum_left += size_left
      my_coords = ( [(time_begin, bottom_left), (time_begin, csum_left), \
  ...
  def get_coords( self ):
    try:
      ...
      for idx in range(wedges_len):
        ...
        coords[ orig_label ] = tuple( [(i[0],height-i[1]) for i in v] )
      self.coords = coords
      return coords
    except:
      return None
